...
/*###############################################
#  設定系統變數值，一般更新 options 的值
#  set_modules_system_var($tbl,$kind,$name,$value,$key = "options");
#  (資料表,類別,欄名,資料庫欄名)#
#  給前台使用，只撈 enable=1
#################################################*/
if (!function_exists("set_modules_system_var")) {
	function set_modules_system_var($tbl,$kind,$name,$value,$key = "options") {
		global $xoopsDB;
		if (empty($tbl) or empty($kind) or empty($name) or empty($value)) {
			return;
    }
    #更新系統變數值
...
function ajaxDebug($content,$name=""){
  ...
  Utility::mk_dir(XOOPS_ROOT_PATH . "/uploads/debug");
