...
if (!function_exists("ugm_module_system_list")) {
	function ugm_module_system_list($kind = "config", $name = "") {
		global $xoopsDB,$xoopsTpl,$tbl,$kind_arr,$module;
  ...
  ";
		...
		while ($row = $xoopsDB->fetchArray($result)) {
			...
			if ($row['formtype'] == "textbox" or $row['formtype'] == "textarea") {
			  ...
			  #----單檔圖片上傳
			  //$module['dirname'] = $module['dirname'];       //專案名稱
			  $subdir     = "system";       //子目錄
			  $ugmUpFiles = new ugmUpFiles($module['dirname'], $subdir);//實體化
			  ...
			  if($row['value']){				
				...
				$row['options'] = array_flip(json_decode($row['options'], true));
				$row['value'] = $myts->htmlSpecialChars($row['options'][$row['value']]);
			}
...
if (!function_exists("ugm_module_system_insert")) {
	function ugm_module_system_insert() {
		global $xoopsDB,$xoopsUser,$tbl,$module,$xoopsConfig;
		...
		if ($row['formtype'] == "file") {
		    ...
		    //$module['dirname'] = $module['dirname'];             //專案名稱
		    $subdir = "system"; //子目錄
		    $ugmUpFiles = new ugmUpFiles($module['dirname'], $subdir);
		    ...
		    #------------------------------------------------
				...
				$col_name = $row['name'];
				$ugmUpFiles = new ugmUpFiles($module['dirname'], $dir_name, NULL, "", "/thumbs", $multiple);
...
/*##############################################
#  編輯表單
$module => modules/interface_menu.php

$module = [];
$module['mid'] = $xoopsModule->mid();//模組id
$module['name'] = $xoopsModule->name();//模組中文名稱
$module['dirname'] =  $xoopsModule->dirname();//模組英文名稱
$module['isAdmin'] = $xoopsUser ? $xoopsUser->isAdmin($module['mid']) : false;

##############################################*/
if (!function_exists("ugm_module_system_form")) {
	function ugm_module_system_form($sn = "", $kind = "system", $name = "") {
		global $xoopsDB,$xoopsUser,$tbl,$xoopsTpl,$module,$kind_arr;
		//----------------------------------*/
		//抓取預設值
		if (!empty($sn)) {
		...
		if ($row['formtype'] == "file") {
			...
			if ($row['valuetype'] == "single_img") {
			    ...
			    #----單檔圖片上傳
			    //$module['dirname'] = $module['dirname']; //模組名稱
			    $subdir = "system";                          //子目錄
			    $ugmUpFiles = new ugmUpFiles($module['dirname'], $subdir);//實體化 
			...
			#---- 檢查資料夾
			Utility::mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/fck");
			Utility::mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/image");
			Utility::mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/fck/flash");

			$dir_name = $module['dirname'] . "/fck";
			$ck = new CKEditor($dir_name, "value", $row['value'], $module['dirname']);
			$ck->setHeight(300);
			//pinrt_r($ck);die();
			$row['form'] = $ck->render();
