...
#引入
require_once XOOPS_ROOT_PATH . "/modules/ugm_tools2/ugmTools.php";
#----檢查類
// 檢查資料表是否存在(資料表)
...
/*
 #---- 變更型態
  $tbl="ugm_a_cart_prod_standard";
  $col="sprice";
  $type="decimal(7,2) unsigned";//int(10) unsigned
  if(!check_columnType($type,$col,$tbl)){
      $sql="ALTER 
      TABLE `".$xoopsDB->prefix($tbl)."` 
      MODIFY {$col} 
      decimal(7,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '特價';
    ";
    $xoopsDB->queryF($sql);
  }
*/
if(!function_exists("check_columnType")){
	function check_columnType($type,$col,$tbl){
		global $xoopsDB;
		if (!$type or !$col or !$tbl) {
			return false;
		}
		$row = getColumnType($col,$tbl);
		//ajaxDebug($row['Type']."\n".$type,"check_columnType");
		if($row['Type'] == $type){			
		  return true;
		}
	  return false;
...
/*=============================
	...
	$ver = 1.20
=============================*/
if(!function_exists("check_columnDefault")){
	function check_columnDefault($default,$col,$tbl){
		global $xoopsDB;
		if (!$col or !$tbl) {
			return false;
		}
		$row = getColumnType($col,$tbl);
		//ajaxDebug($row['Default']."\n".$default,"check_columnDefault");
