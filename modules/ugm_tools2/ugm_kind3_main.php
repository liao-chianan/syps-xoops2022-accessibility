...
//$module['dirname'] = $xoopsModule->getVar('dirname');
#實體化 類別物件
$stopLevel = $foreign[$kind]['stopLevel']; //層數

#(模組名稱,關鍵字，層數)
$ugmKind = new ugmKind($module['dirname'],$kind,$stopLevel);
# 如果資料表非預設 請自行設定
# $ugmKind -> set_tbl($tbl)
/*------------------------*/
...
case "opUpdateSort": //更新排序
  ...
  echo opUpdateSort();
  transaction($module['dirname'],$kind,$stopLevel);
  //XoopsCache::clear();
  exit;
...
$xoTheme->addStylesheet(XOOPS_URL . "/modules/{$module['dirname']}/css/module.css");
...
$xoopsTpl->assign("action", XOOPS_URL . "/modules/{$module['dirname']}/{$file_name}");
...
###########################################################
function opForm($sn = "") {
  global $xoopsDB,$xoopsTpl,$ugmKind,$foreign,$forms,$formsSwitch,$module;
  ...
  if(isset($formsSwitch['content'])){
    ...
    if($type == "fck"){
      ...
      #---- 檢查資料夾
      mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}");
      mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/fck");
      mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/fck/image");
      mk_dir(XOOPS_ROOT_PATH . "/uploads/{$module['dirname']}/fck/flash");
      $dir_name = $module['dirname'] . "/fck";
      #----
      $ck = new CKEditor($dir_name, "content", $row['content'], $module['dirname']);
      $ck->setHeight(200);
