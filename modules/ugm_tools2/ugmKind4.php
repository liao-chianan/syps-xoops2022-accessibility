...
class ugmKind {
	...
	public function get_listArr($ofsn=0,$level=1,$enable=0) {
		...
		while ($row =  $xoopsDB->fetchArray($result)) {
			...
			//移動圖示
			$icon['move_i'] = ($icon['folder_i'] or $this->stopLevel == '1') ? false:true;
			//增加類別圖示
	...
	#  
	################################################################
	public function get_rowsHtml($rows,$listHead) {
		$html = "";	
		foreach ($rows as $item => $row) {
			$html .= $this->get_rowHtml($row,$listHead);
			if ($row['sub']) {
				$html .= $this->get_rowsHtml($row['sub'],$listHead);
			}
		}
		return $html;
	}
	...
	################################################################
	public function get_listHtml($rows,$listHead) {
		#標題
		$thHtml = "";
		foreach($listHead as $col => $v){
				$thHtml .= "<th{$v['th']['attr']}>{$v['th']['title']}</th>";
		}

		#內容
		$rowsHtml = $this->get_rowsHtml($rows,$listHead);
				...
				<table id='form_table' class='table table-bordered table-striped table-hover table-condensed'>
				  ...
				  <tr id='tr_0' data-tt-id='0'>
				    <td class='text-left' colspan={$count}>
				      ...
				      <a href='#' class='btn btn-success btn-sm' onclick=\"jQuery('#form_table').treetable('expandAll'); return false;\">展開<i class='fa fa-expand' aria-hidden='true'></i></a>

				      <a href='#' class='btn btn-danger btn-sm' onclick=\"jQuery('#form_table').treetable('collapseAll'); return false;\">闔起<i class='fa fa-compress' aria-hidden='true'></i></a>

				      <a href='?op=opForm&kind={$this->kind}&ofsn=0' class='btn btn-primary btn-sm' ait='在根目錄建立子類別'>新增<i class='fa fa-plus' aria-hidden='true'></i></a>
				    </td>
				  </tr>
				  <!-- 根目錄結束 -->
	...
	#  data-tt-parent-id 父層
	################################################################
	public function get_rowHtml($row,$listHead) {		
		#row自己的層數
		...
		foreach ($listHead as $col => $table){
          ...
          <a href='?op=opForm&ofsn={$row['sn']}&kind={$row['kind']}' title='IN ({$row['title']}) 建立子類別' class='btn btn-primary btn-sm'>
			  ...
			  if($row[$col]){				
				...
				if ($row[$col] == 1) {
					#啟用
					$target_0 = "本站";
					$html .= "<a href='?op=opUpdateTarget&sn={$row['sn']}&target=0&kind=" . $this->get_kind() . "' title='{$target_0}' atl='{$target_0}' class='btn btn-success btn-sm'><i class='fa fa-check' aria-hidden='true'></i></a>";
				} else {
					#停用
					$target_1 = "外連";
					$html .= "<a href='?op=opUpdateTarget&sn={$row['sn']}&target=1&kind=" . $this->get_kind() . "' title='{$target_1}' atl='{$target_1}' class='btn btn-danger btn-sm'><i class='fa fa-times' aria-hidden='true'></i></a>";
				}
				...
				if ($row[$col] == 1) {
					#啟用 
					$enable_0 = "停用";
					$html .= "<a href='?op=opUpdateEnable&sn={$row['sn']}&enable=0&kind=" . $this->get_kind() . "' title='{$enable_0}' atl='{$enable_0}' class='btn btn-success btn-sm'><i class='fa fa-check' aria-hidden='true'></i></a>";

				} else {
					#停用
					$enable_1 = "啟用";
					$html .= "<a href='?op=opUpdateEnable&sn={$row['sn']}&enable=1&kind=" . $this->get_kind() . "' title='{$enable_1}' atl='{$enable_1}' class='btn btn-danger btn-sm'><i class='fa fa-times' aria-hidden='true'></i></a>";
				...
				foreach ($table['td']['btn'] as $btn) {
					if ($btn == "view") {
						...
						$html .= "<a href='?op=opForm&sn={$row['sn']}&kind=" . $this->get_kind() . "' class='btn btn-success btn-sm'>"._EDIT."</a> ";

					} elseif ($btn == "del") {
						$disable = $downLevel ?" disabled":"";
						$html .= "<button type='button' class='btn btn-sm btn-danger btnDel{$disable}'>刪除</button> ";
	...
	#  $level 取幾層
	################################################*/
	public function get_kindArr($ofsn=0,$level=1,$enable=1) {
		global $xoopsDB;
		...
		while ($row =  $xoopsDB->fetchArray($result)) {
			...
			/*------------------------------------*/
			#子類別記錄			
			$row['sub'] = $this->get_kindArr($row['sn'], $downLevel,$enable);
			$rows[] = $row;
		}
		return $rows;
	}
