...
//        }
                ...
                foreach ($rels->Relationship as $rel) {
                    switch ($rel['Type']) {
                        case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                            if ('workbook.xml' == basename($rel['Target'])) {
                                $xl = true;
                            }
                            break;
                    }
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument' == $rel['Type']) {
                ...
                foreach ($relsWorkbook->Relationship as $ele) {
                    if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet' == $ele['Type']) {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $currCells = 0;
                        while ($xml->read()) {
                            if ('row' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                                $row = $xml->getAttribute('r');
                                $tmpInfo['totalRows'] = $row;
                                $tmpInfo['totalColumns'] = max($tmpInfo['totalColumns'], $currCells);
                                $currCells = 0;
                            } elseif ('c' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                                $currCells++;
                            }
                        }
                        $tmpInfo['totalColumns'] = max($tmpInfo['totalColumns'], $currCells);
...
//        echo '$c->f is ', $c->f, PHP_EOL;
        ...
        // Shared formula?
        if (isset($c->f['t']) && 'shared' == mb_strtolower((string)$c->f['t'])) {
...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    private function getFromZipArchive($archive, $fileName = '')
    {
        ...
        if (false === $contents) {
            $contents = $archive->getFromIndex(
                $archive->locateName(mb_substr($fileName, 1), ZIPARCHIVE::FL_NOCASE)
            );
        }
                    ...
                    if (!$this->readDataOnly && $xmlStyles) {
                        ...
                        if ($xmlStyles->cellStyles) {
                            foreach ($xmlStyles->cellStyles->cellStyle as $cellStyle) {
                                if (0 == intval($cellStyle['builtinId'])) {
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (isset($xmlSheet->sheetFormatPr)) {
                                ...
                                if (isset($xmlSheet->sheetFormatPr['zeroHeight']) &&
                                    ('1' == (string)$xmlSheet->sheetFormatPr['zeroHeight'])) {
...
//                                                echo 'Default', PHP_EOL;
                                        ...
                                        if (is_numeric($value) && 's' != $cellDataType) {
                                            if ($value == (int)$value) {
                                                $value = (int)$value;
                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                foreach ($xmlSheet->conditionalFormatting as $conditional) {
                                    foreach ($conditional->cfRule as $cfRule) {
                                        if ((PHPExcel_Style_Conditional::CONDITION_NONE == (string)$cfRule['type'] || PHPExcel_Style_Conditional::CONDITION_CELLIS == (string)$cfRule['type'] || PHPExcel_Style_Conditional::CONDITION_CONTAINSTEXT == (string)$cfRule['type'] || PHPExcel_Style_Conditional::CONDITION_EXPRESSION == (string)$cfRule['type']) && isset($dxfs[intval($cfRule['dxfId'])])) {
                            ...
                            if ($xmlSheet && $xmlSheet->autoFilter && !$this->readDataOnly) {
                                ...
                                if (false !== mb_strpos($autoFilterRange, ':')) {
                                    ...
                                    foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                        ...
                                        if ($filterColumn->filters) {
                                            ...
                                            if ((isset($filters['blank'])) && (1 == $filters['blank'])) {
                                        ...
                                        if ($filterColumn->customFilters) {
                                            ...
                                            if ((isset($customFilters['and'])) && (1 == $customFilters['and'])) {
                                        ...
                                        if ($filterColumn->top10) {
                                            ...
                                            foreach ($filterColumn->top10 as $filterRule) {
                                                $column->createRule()->setRule(
                                                    (
                                                        ((isset($filterRule['percent'])) && (1 == $filterRule['percent']))
                                                    ...
                                                    (string) $filterRule['val'],
                                                    (
                                                        ((isset($filterRule['top'])) && (1 == $filterRule['top']))
                                ...
                                foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
                                    ...
                                    foreach ($rangeSet as $range) {
                                        ...
                                        foreach (PHPExcel_Cell::extractAllCellReferencesInRange($stRange) as $reference) {
                                            ...
                                            $docValidation->setAllowBlank(0 != $dataValidation['allowBlank']);
                                            $docValidation->setShowDropDown(0 == $dataValidation['showDropDown']);
                                            $docValidation->setShowInputMessage(0 != $dataValidation['showInputMessage']);
                                            $docValidation->setShowErrorMessage(0 != $dataValidation['showErrorMessage']);
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                    ...
                                    foreach ($relsWorksheet->Relationship as $ele) {
                                        if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink' == $ele['Type']) {
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                    $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                    foreach ($relsWorksheet->Relationship as $ele) {
                                        if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments' == $ele['Type']) {
                                            $comments[(string)$ele['Id']] = (string)$ele['Target'];
                                        }
                                        if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing' == $ele['Type']) {
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    foreach ($shapes as $shape) {
                                        ...
                                        if (isset($shape['style'])) {
                                            ...
                                            $fillColor = mb_strtoupper(mb_substr((string)$shape['fillcolor'], 1));
                                            $column = null;
                                            $row = null;
                                            ...
                                            if (is_array($clientData) && !empty($clientData)) {
                                                ...
                                                if (isset($clientData['ObjectType']) && 'Note' == (string)$clientData['ObjectType']) {
                                            ...
                                            if ((null !== $column) && (null !== $row)) {
                                                ...
                                                foreach ($styleArray as $stylePair) {
                                                    ...
                                                    if ('margin-left' == $stylePair[0]) {
                                                        $comment->setMarginLeft($stylePair[1]);
                                                    }
                                                    if ('margin-top' == $stylePair[0]) {
                                                        $comment->setMarginTop($stylePair[1]);
                                                    }
                                                    if ('width' == $stylePair[0]) {
                                                        $comment->setWidth($stylePair[1]);
                                                    }
                                                    if ('height' == $stylePair[0]) {
                                                        $comment->setHeight($stylePair[1]);
                                                    }
                                                    if ('visibility' == $stylePair[0]) {
                                                        $comment->setVisible('visible' == $stylePair[1]);
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                        ...
                                        if ('' != $vmlRelationship) {
                                            ...
                                            foreach ($relsVML->Relationship as $ele) {
                                                if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/image' == $ele['Type']) {
                            ...
                            if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                ...
                                $drawings = [];
                                foreach ($relsWorksheet->Relationship as $ele) {
                                    if ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing' == $ele['Type']) {
                                    ...
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        if ($relsDrawing && $relsDrawing->Relationship) {
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                ...
                                                } elseif ('http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart' == $ele['Type']) {
                            ...
                            if ($xmlWorkbook->definedNames) {
                                foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                    ...
                                    if (false !== ($spos = mb_strpos($extractedRange, '!'))) {
                                        $extractedRange = mb_substr($extractedRange, 0, $spos) . str_replace('$', '', mb_substr($extractedRange, $spos));
                                    } else {
                                        $extractedRange = str_replace('$', '', $extractedRange);
                                    }

                                    // Valid range?
                                    if (false !== mb_stripos((string)$definedName, '#REF!') || '' == $extractedRange) {
                                        continue;
                                    }
                                    ...
                                    if ('' != (string)$definedName['localSheetId'] && (string)$definedName['localSheetId'] == $sheetId) {
                                        ...
                                        switch ((string)$definedName['name']) {
                                            case '_xlnm._FilterDatabase':
                                                if ('1' !== (string)$definedName['hidden']) {
                                ...
                                if ('' != (string)$definedName['localSheetId']) {
                                    ...
                                    switch ((string)$definedName['name']) {
                                        ...
                                        case '_xlnm.Print_Area':
                                            break;
                                        default:
                                            if (null !== $mapSheetId[(int) $definedName['localSheetId']]) {
                                    ...
                                    $locatedSheet = null;
                                    $extractedSheetName = '';
                                    if (false !== mb_strpos((string)$definedName, '!')) {
                                        // Extract sheet name
                    ...
                    if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
                        ...
                        if (isset($mapSheetId[$activeTab]) && null !== $mapSheetId[$activeTab]) {
                            $excel->setActiveSheetIndex($mapSheetId[$activeTab]);
                        } else {
                            if (0 == $excel->getSheetCount()) {
                                $excel->createSheet();
                            }
...
//        }
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->vertAlign) && isset($style->font->vertAlign['val'])) {
                ...
                if ('superscript' == $vertAlign) {
                    $docStyle->getFont()->setSuperScript(true);
                }
                if ('subscript' == $vertAlign) {
