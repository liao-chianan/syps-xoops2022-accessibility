...
//        }
        ...
        if (true === $zip->open($pFilename)) {
            ...
            return ('application/vnd.oasis.opendocument.spreadsheet' === $mimeType);
        }
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        while ($xml->read()) {
            ...
            while ($xml->read()) {
                if ('table:table' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                    //    Loop through each table:table node reading the table:name attribute for each worksheet name
                    do {
                        $worksheetNames[] = $xml->getAttribute('table:name');
                        $xml->next();
                    } while ('table:table' == $xml->name && XMLReader::ELEMENT == $xml->nodeType);
                }
            }
        }
        ...
        while ($xml->read()) {
            ...
            while ($xml->read()) {
                if ('table:table' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                    ...
                    do {
                        $xml->read();
                        if ('table:table-row' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                            ...
                            $xml->read();
                            do {
                                if ('table:table-cell' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                                    if (!$xml->isEmptyElement) {
                                        $currCells++;
                                        $xml->next();
                                    } else {
                                ...
                                } elseif ('table:covered-table-cell' == $xml->name && XMLReader::ELEMENT == $xml->nodeType) {
                                    $mergeSize = $xml->getAttribute('table:number-columns-repeated');
                                    $currCells += $mergeSize;
                                    $xml->read();
                                }
                            } while ('table:table-row' != $xml->name);
                        }
                    } while ('table:table' != $xml->name);
...
//                                    echo 'Value Type is '.$cellDataOfficeAttributes['value-type'].'<br />';
                                    switch ($cellDataOfficeAttributes['value-type']) {
                                        ...
                                        case 'boolean':
                                            ...
                                            $dataValue = ('TRUE' == $allCellDataText) ? true : false;
                                            break;
                                        case 'percentage':
...
//                                    echo 'Formula: ', $cellDataFormula, PHP_EOL;
                                    $cellDataFormula = mb_substr($cellDataFormula, mb_strpos($cellDataFormula, ':=') + 1);
...
//                                    echo 'Adjusted Formula: ', $cellDataFormula, PHP_EOL;
                                ...
                                if (null !== $type) {
                                    for ($i = 0; $i < $colRepeats; ++$i) {
                                        ...
                                        if (PHPExcel_Cell_DataType::TYPE_NULL !== $type) {
...
//                                                    echo 'Forumla result is '.$dataValue.'<br />';
                                ...
                                if ((isset($cellDataTableAttributes['number-columns-spanned'])) || (isset($cellDataTableAttributes['number-rows-spanned']))) {
                                    if ((PHPExcel_Cell_DataType::TYPE_NULL !== $type) || (!$this->readDataOnly)) {
                                        $columnTo = $columnID;
