...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    protected function isValidFormat()
    {
        ...
        $delimiterCount = mb_substr_count($data, ';');
        if ($delimiterCount < 1) {
            return false;
        }
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        $rowIndex = 0;
        while (false !== ($rowData = fgets($fileHandle))) {
            $columnIndex = 0;
            ...
            if ('C' == $dataType) {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'C':
                        case 'X':
                            $columnIndex = mb_substr($rowDatum, 1) - 1;
                            break;
                        case 'R':
                        case 'Y':
                            $rowIndex = mb_substr($rowDatum, 1);
                            break;
                    }
        ...
        while (false !== ($rowData = fgets($fileHandle))) {
            ...
            if ('P' == $dataType) {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'P':
                            $formatArray['numberformat']['code'] = str_replace($fromFormats, $toFormats, mb_substr($rowDatum, 1));
                            break;
                        case 'E':
                        case 'F':
                            $formatArray['font']['name'] = mb_substr($rowDatum, 1);
                            break;
                        case 'L':
                            $formatArray['font']['size'] = mb_substr($rowDatum, 1);
                            break;
                        case 'S':
                            $styleSettings = mb_substr($rowDatum, 1);
                            for ($i = 0; $i < mb_strlen($styleSettings); ++$i) {
            ...
            //    Read cell value data
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'C':
                        case 'X':
                            $column = mb_substr($rowDatum, 1);
                            break;
                        case 'R':
                        case 'Y':
                            $row = mb_substr($rowDatum, 1);
                            break;
                        case 'K':
                            $cellData = mb_substr($rowDatum, 1);
                            break;
                        case 'E':
                            $cellDataFormula = '=' . mb_substr($rowDatum, 1);
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = !$key) {
                                    ...
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        $value = substr_replace($value, $A1CellReference, $cellReference[0][1], mb_strlen($cellReference[0][0]));
                                    }
                                }
                            }
                            unset($value);
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        ...
                        case 'P':
                            ...
                            break;
                        case 'W':
                            list($startCol, $endCol, $columnWidth) = explode(' ', mb_substr($rowDatum, 1));
                            break;
                        case 'S':
                            $styleSettings = mb_substr($rowDatum, 1);
                            for ($i = 0; $i < mb_strlen($styleSettings); ++$i) {
