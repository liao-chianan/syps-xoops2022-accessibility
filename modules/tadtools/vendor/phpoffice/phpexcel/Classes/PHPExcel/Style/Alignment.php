...
class PHPExcel_Style_Alignment extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    public function setTextRotation($pValue = 0)
    {
        ...
        // Set rotation
        if (($pValue >= -90 && $pValue <= 90) || -165 == $pValue) {
            if ($this->isSupervisor) {
    ...
    {
        if ($pValue > 0) {
            if (self::HORIZONTAL_GENERAL != $this->getHorizontal() &&
                self::HORIZONTAL_LEFT != $this->getHorizontal() &&
                self::HORIZONTAL_RIGHT != $this->getHorizontal()) {
