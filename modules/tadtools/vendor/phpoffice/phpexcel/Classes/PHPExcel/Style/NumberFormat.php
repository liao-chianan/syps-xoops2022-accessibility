...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    public function getFormatCode()
    {
        ...
        if (false !== $this->builtInFormatCode) {
    ...
    private static function formatAsPercentage(&$value, &$format)
    {
        if (self::FORMAT_PERCENTAGE === $format) {
            ...
            if (preg_match('/\.[#0]+/i', $format, $m)) {
                $s = mb_substr($m[0], 0, 1) . (mb_strlen($m[0]) - 1);
                $format = str_replace($m[0], $s, $format);
            }
            if (preg_match('/^[#0]+/', $format, $m)) {
                $format = str_replace($m[0], mb_strlen($m[0]), $format);
            }
    ...
    private static function formatAsFraction(&$value, &$format)
    {
        ...
        $decimalLength = mb_strlen($decimalPart);
        ...
        if ((false !== mb_strpos($format, '0')) || (false !== mb_strpos($format, '#')) || ('? ?' == mb_substr($format, 0, 3))) {
            if (0 == $integerPart) {
                $integerPart = '';
            }
    ...
    public static function toFormattedString($value = '0', $format = self::FORMAT_GENERAL, $callBack = null)
    {
        ...
        if ((self::FORMAT_GENERAL === $format) || (self::FORMAT_TEXT === $format)) {
            return $value;
        }
        ...
        if (preg_match('/(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy](?=(?:[^"]|"[^"]*")*$)/miu', $format, $matches)) {
            ...
            if (self::FORMAT_CURRENCY_EUR_SIMPLE === $format) {
                ...
                if (preg_match('/(#|0)(,+)/', $format, $matches)) {
                    $scale = pow(1000, mb_strlen($matches[2]));
                ...
                if (preg_match('/#?.*\?\/\?/', $format, $m)) {
                    ...
                    if (preg_match($number_regex, $m, $matches)) {
                        ...
                        $minWidth = mb_strlen($left) + mb_strlen($dec) + mb_strlen($right);
                        if ($useThousands) {
                            $value = number_format(
                                $value,
                                mb_strlen($right),
                            ...
                            if (preg_match('/[0#]E[+-]0/i', $format)) {
                                ...
                                $sprintf_pattern = "%0$minWidth." . mb_strlen($right) . 'f';
