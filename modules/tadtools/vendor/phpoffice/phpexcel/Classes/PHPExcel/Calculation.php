...
class PHPExcel_Calculation
{
    ...
    private static function loadLocales()
    {
        ...
        foreach (glob($localeFileDirectory . '/*', GLOB_ONLYDIR) as $filename) {
            $filename = mb_substr($filename, mb_strlen($localeFileDirectory) + 1);
            if ('en' != $filename) {
    ...
    {
        if ((self::RETURN_ARRAY_AS_VALUE == $returnType) ||
            (self::RETURN_ARRAY_AS_ERROR == $returnType) ||
            (self::RETURN_ARRAY_AS_ARRAY == $returnType)) {
    ...
    public function setLocale($locale = 'en_us')
    {
        ...
        if (1 == count(self::$validLocaleLanguages)) {
            self::loadLocales();
        }
        ...
        if (in_array($language, self::$validLocaleLanguages, true)) {
            ...
            if ('en_us' != $locale) {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (false !== mb_strpos($localeFunction, '=')) {
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        ...
                        if (false !== mb_strpos($localeSetting, '=')) {
    ...
    private static function translateFormula($from, $to, $formula, $fromSeparator, $toSeparator)
    {
        ...
        if ('en_us' !== self::$localeLanguage) {
            $inBraces = false;
            //    If there is the possibility of braces within a quoted string, then we don't treat those as matrix indicators
            if (false !== mb_strpos($formula, '"')) {
    ...
    {
        if (null === self::$functionReplaceFromExcel) {
        ...
        if (null === self::$functionReplaceToLocale) {
    ...
    {
        if (null === self::$functionReplaceFromLocale) {
        ...
        if (null === self::$functionReplaceToExcel) {
    ...
    {
        if (is_string($value)) {
            if ((isset($value[0])) && ('"' == $value[0]) && ('"' == mb_substr($value, -1))) {
    ...
    public function calculateCellValue(PHPExcel_Cell $pCell = null, $resetLog = true)
    {
        ...
        if ((is_array($result)) && (self::RETURN_ARRAY_AS_ARRAY != self::$returnArrayAsType)) {
            self::$returnArrayAsType = $returnArrayAsType;
            $testResult = PHPExcel_Calculation_Functions::flattenArray($result);
            if (self::RETURN_ARRAY_AS_ERROR == self::$returnArrayAsType) {
                return PHPExcel_Calculation_Functions::VALUE();
            }
            //    If there's only a single cell in the array, then we allow it
            if (1 != count($testResult)) {
    ...
    public function parseFormula($formula)
    {
        ...
        $formula = ltrim(mb_substr($formula, 1));
        if (!isset($formula[0])) {
            return [];
        }
    ...
    public function calculateFormula($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        if (null !== $this->workbook && null === $cellID && null === $pCell) {
            $cellID = 'A1';
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        $pCellParent = (null !== $pCell) ? $pCell->getWorksheet() : null;
        $wsTitle = (null !== $pCellParent) ? $pCellParent->getTitle() : "\x00Wrk";
        $wsCellReference = $wsTitle . '!' . $cellID;

        if ((null !== $cellID) && ($this->getValueFromCache($wsCellReference, $cellValue))) {
            return $cellValue;
        }

        if (("\x00" !== $wsTitle[0]) && ($this->cyclicReferenceStack->onStack($wsCellReference))) {
        ...
        if (false !== mb_strpos($formula, '{')) {
            ...
            if (false !== mb_strpos($formula, '"')) {
                ...
                foreach ($temp as &$value) {
                    //    Only count/replace in alternating array entries
                    if ($i = !$i) {
                        $openCount += mb_substr_count($value, '{');
                        $closeCount += mb_substr_count($value, '}');
                ...
                //    If there's no quoted strings, then we do a simple count/replace
                $openCount = mb_substr_count($formula, '{');
                $closeCount = mb_substr_count($formula, '}');
    ...
    {
        if (false === ($formula = $this->convertMatrixReferences(trim($formula)))) {
            return false;
        }
...
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (mb_strlen($formula) > $index) && (isset(self::$comparisonOperators[$formula[$index + 1]]))) {
                $opCharacter .= $formula[++$index];
                //echo 'Initial character of expression block is comparison operator '.$opCharacter.PHP_EOL;
            }

            //    Find out if we're currently at the beginning of a number, variable, cell reference, function, parenthesis or operand
            $isOperandOrFunction = preg_match($regexpMatchString, mb_substr($formula, $index), $match);
...
//                    echo 'Element '.$val.' is a Function<br />';
                    if (isset(self::$PHPExcelFunctions[mb_strtoupper($matches[1])]) || isset(self::$controlFunctions[mb_strtoupper($matches[1])])) {    // it's a function
                        ...
                        $ax = preg_match('/^\s*(\s*\))/ui', mb_substr($formula, $index + $length), $amatch);
                        if ($ax) {
...
//                    echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
                    ...
                    if (':' == $testPrevOp['value']) {
                        ...
                        $rangeWS1 = '';
                        if (false !== mb_strpos('!', $startRowColRef)) {
                        ...
                        if ((is_int($startRowColRef)) && (ctype_digit($val)) &&
                            ...
                            //    Row range
                            $endRowColRef = (null !== $pCellParent) ? $pCellParent->getHighestColumn() : 'XFD';    //    Max 16,384 columns for Excel2007
                            $output[count($output) - 1]['value'] = $rangeWS1 . 'A' . $startRowColRef;
                            $val = $rangeWS2 . $endRowColRef . $val;
                        } elseif ((ctype_alpha($startRowColRef)) && (ctype_alpha($val)) &&
                            (mb_strlen($startRowColRef) <= 3) && (mb_strlen($val) <= 3)) {
                            //    Column range
                            $endRowColRef = (null !== $pCellParent) ? $pCellParent->getHighestRow() : 1048576;        //    Max 1,048,576 rows for Excel2007
...
//                        echo 'Element is a Number<br />';
                        if ((false !== mb_strpos($val, '.')) || (false !== mb_stripos($val, 'e')) || ($val > PHP_INT_MAX) || ($val < -PHP_INT_MAX)) {
...
//                        echo 'Element '.$excelConstant.' is an Excel Constant<br />';
                    ...
                    } elseif (false !== ($localeConstant = array_search(trim(mb_strtoupper($val)), self::$localeBoolean, true))) {
...
//                echo 'Possible Intersect Operator<br />';
                if (($expectingOperator) && (preg_match('/^' . self::CALCULATION_REGEXP_CELLREF . '.*/Ui', mb_substr($formula, $index), $match)) &&
                    ('Cell Reference' == $output[count($output) - 1]['type'])) {
//                    echo 'Element is an Intersect Operator<br />';
    ...
    private static function dataTestReference(&$operandData)
    {
        ...
        if ((null === $operandData['reference']) && (is_array($operand))) {
    ...
    private function processTokenStack($tokens, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        $pCellWorksheet = (null !== $pCell) ? $pCell->getWorksheet() : null;
        $pCellParent = (null !== $pCell) ? $pCell->getParent() : null;
        $stack = new PHPExcel_Calculation_Token_Stack();
...
//                echo 'Token is a binary operator<br />';
                ...
                if (null === ($operand2Data = $stack->pop())) {
                    return $this->raiseFormulaError('Internal error - Operand value missing from stack');
                }
                if (null === ($operand1Data = $stack->pop())) {
                    return $this->raiseFormulaError('Internal error - Operand value missing from stack');
                }
                ...
                switch ($token) {
                    ...
                    case ':':            //    Range
                        $sheet1 = $sheet2 = '';
                        if (false !== mb_strpos($operand1Data['reference'], '!')) {
                            list($sheet1, $operand1Data['reference']) = explode('!', $operand1Data['reference']);
                        } else {
                            $sheet1 = (null !== $pCellParent) ? $pCellWorksheet->getTitle() : '';
                        }
                        if (false !== mb_strpos($operand2Data['reference'], '!')) {
                            list($sheet2, $operand2Data['reference']) = explode('!', $operand2Data['reference']);
                        } else {
                            $sheet2 = $sheet1;
                        }
                        if ($sheet1 == $sheet2) {
                            if (null === $operand1Data['reference']) {
                            ...
                            if (null === $operand2Data['reference']) {
...
//                        We can't access the range, so return a REF error
                        ...
                        if ($matches[2] > '') {
                            ...
                            if ((false !== mb_strpos($matches[2], '[')) || (false !== mb_strpos($matches[2], ']'))) {
...
//                echo 'Token is a function<br />';
                ...
                if ('MKMATRIX' != $functionName) {
                    $this->_debugLog->writeDebugLog('Evaluating Function ', self::localeFunc($functionName), '() with ', ((0 == $argCount) ? 'no' : $argCount), ' argument', ((1 == $argCount) ? '' : 's'));
                }
...
//                    } else {
                    ...
                    if (false !== mb_strpos($functionCall, '::')) {
...
//                    echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
                ...
                } elseif ((is_numeric($token)) || (null === $token) || (is_bool($token)) || ('' == $token) || ('"' == $token[0]) || ('#' == $token[0])) {
...
//        }
    ...
    {
        if (is_array($operand)) {
            if (1 == (count($operand, COUNT_RECURSIVE) - count($operand))) {
                do {
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        $useLowercaseFirstComparison = is_string($operand1) && is_string($operand2) && PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE == PHPExcel_Calculation_Functions::getCompatibilityMode();
    ...
    private function executeNumericBinaryOperation($cellID, $operand1, $operand2, $operation, $matrixFunction, &$stack)
    {
        ...
        if ((is_array($operand1)) || (is_array($operand2))) {
            ...
            if ((PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE != PHPExcel_Calculation_Functions::getCompatibilityMode()) &&
                ((is_string($operand1) && !is_numeric($operand1) && mb_strlen($operand1) > 0) ||
                 (is_string($operand2) && !is_numeric($operand2) && mb_strlen($operand2) > 0))) {
...
//            echo '<br />';
    ...
    public function isImplemented($pFunction = '')
    {
        ...
        if (isset(self::$PHPExcelFunctions[$pFunction])) {
            return ('PHPExcel_Calculation_Functions::DUMMY' != self::$PHPExcelFunctions[$pFunction]['functionCall']);
        }
    ...
    public function listFunctions()
    {
        ...
        foreach (self::$PHPExcelFunctions as $functionName => $function) {
            if ('PHPExcel_Calculation_Functions::DUMMY' != $function['functionCall']) {
