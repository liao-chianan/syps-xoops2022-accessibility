...
class PHPExcel_ReferenceHelper
{
    ...
    {
        return strcasecmp(mb_strlen($a) . $a, mb_strlen($b) . $b);
    }
    ...
    {
        return 1 - strcasecmp(mb_strlen($a) . $a, mb_strlen($b) . $b);
    }
    ...
    public static function cellSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return strcasecmp(mb_strlen($ac) . $ac, mb_strlen($bc) . $bc);
        }
    ...
    public static function cellReverseSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return 1 - strcasecmp(mb_strlen($ac) . $ac, mb_strlen($bc) . $bc);
        }
    ...
    public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExcel_Worksheet $pSheet = null)
    {
        ...
        while ($cellID = array_pop($aCellCollection)) {
            ...
            if (($cellIndex >= $beforeColumnIndex) && ($cell->getRow() >= $beforeRow)) {
                ...
                if (PHPExcel_Cell_DataType::TYPE_FORMULA == $cell->getDataType()) {
            ...
            if ($i = !$i) {
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        ...
                        $modified3 = mb_substr($this->updateCellReference('$A' . $match[3], $pBefore, $pNumCols, $pNumRows), 2);
                        $modified4 = mb_substr($this->updateCellReference('$A' . $match[4], $pBefore, $pNumCols, $pNumRows), 2);
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        $fromString = ($match[2] > '') ? $match[2] . '!' : '';
                        $fromString .= $match[3] . ':' . $match[4];
                        $modified3 = mb_substr($this->updateCellReference($match[3] . '$1', $pBefore, $pNumCols, $pNumRows), 0, -2);
                        $modified4 = mb_substr($this->updateCellReference($match[4] . '$1', $pBefore, $pNumCols, $pNumRows), 0, -2);
    ...
    public function updateCellReference($pCellRange = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        ...
        if (false !== mb_strpos($pCellRange, '!')) {
            return $pCellRange;
        // Is it a range or a single cell?
        } elseif (false === mb_strpos($pCellRange, ':') && false === mb_strpos($pCellRange, ',')) {
            // Single cell
    ...
    public function updateNamedFormulas(PHPExcel $pPhpExcel, $oldName = '', $newName = '')
    {
        ...
        foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
            foreach ($sheet->getCellCollection(false) as $cellID) {
                ...
                if ((null !== $cell) && (PHPExcel_Cell_DataType::TYPE_FORMULA == $cell->getDataType())) {
                    $formula = $cell->getValue();
                    if (false !== mb_strpos($formula, $oldName)) {
    ...
    {
        if (false !== mb_strpos($pCellRange, ':') || false !== mb_strpos($pCellRange, ',')) {
            // Update range
    ...
    {
        if (false === mb_strpos($pCellReference, ':') && false === mb_strpos($pCellReference, ',')) {
            // Get coordinates of $pBefore
            list($beforeColumn, $beforeRow) = PHPExcel_Cell::coordinateFromString($pBefore);
