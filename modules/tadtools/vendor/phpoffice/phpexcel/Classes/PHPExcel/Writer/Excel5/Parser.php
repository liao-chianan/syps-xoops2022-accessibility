...
class PHPExcel_Writer_Excel5_Parser
{
    ...
    private function convert($token)
    {
        ...
        // match error codes
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $token) or '#N/A' == $token) {
...
//        }
    ...
    private function cellToRowcol($cell)
    {
        ...
        $expn = mb_strlen($col_ref) - 1;
        $col = 0;
        $col_ref_length = mb_strlen($col_ref);
    ...
    private function advance()
    {
        ...
        $formula_length = mb_strlen($this->formula);
        // eat up white spaces
        if ($i < $formula_length) {
        ...
        switch ($token) {
            ...
            default:
                ...
                if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $token) and !preg_match('/[0-9]/', $this->lookAhead) and (':' != $this->lookAhead) and ('.' != $this->lookAhead) and ('!' != $this->lookAhead)) {
                    ...
                    return $token;
                } elseif (preg_match('/"([^"]|""){0,255}"/', $token) and '"' != $this->lookAhead and (0 == mb_substr_count($token, '"') % 2)) {
    ...
    private function expression()
    {
        ...
        if (preg_match('/"([^"]|""){0,255}"/', $this->currentToken)) {
            ...
            return $result;
        // If it's an error code
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $this->currentToken) or '#N/A' == $this->currentToken) {
