...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    public function save($pFilename = null)
    {
        if (null !== $this->spreadSheet) {
            ...
            if ('php://output' == mb_strtolower($pFilename) || 'php://stdout' == mb_strtolower($pFilename)) {
            ...
            if (true !== $objZip->open($pFilename, $zipOverWrite)) {
                if (true !== $objZip->open($pFilename, $zipCreate)) {
            ...
            if ($this->spreadSheet->hasMacros()) {
                ...
                if (null !== $macrosCode) {// we have the code ?
            ...
            for ($i = 0; $i < $this->getDrawingHashTable()->count(); ++$i) {
                if ($this->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    if (false !== mb_strpos($imagePath, 'zip://')) {
                        $imagePath = mb_substr($imagePath, 6);
