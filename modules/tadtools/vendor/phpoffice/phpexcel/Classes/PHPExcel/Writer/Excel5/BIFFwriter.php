...
//        $this->limit      = 8224;
    ...
    {
        if (mb_strlen($data) - 4 > $this->limit) {
            $data = $this->addContinue($data);
        }
        $this->_data .= $data;
        $this->_datasize += mb_strlen($data);
    }
    ...
    private function addContinue($data)
    {
        ...
        $tmp = mb_substr($data, 0, 2) . pack('v', $limit) . mb_substr($data, 4, $limit);
        ...
        for ($i = $limit + 4; $i < ($data_length - $limit); $i += $limit) {
            $tmp .= $header;
            $tmp .= mb_substr($data, $i, $limit);
        }

        // Retrieve the last chunk of data
        $header = pack('vv', $record, mb_strlen($data) - $i);
        $tmp .= $header;
        $tmp .= mb_substr($data, $i, mb_strlen($data) - $i);
