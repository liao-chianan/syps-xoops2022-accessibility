...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
        ...
        if (!empty($arrConditionalStyles)) {
            ...
            foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
                foreach ($conditionalStyles as $conditional) {
                    if (PHPExcel_Style_Conditional::CONDITION_EXPRESSION == $conditional->getConditionType()
                        || PHPExcel_Style_Conditional::CONDITION_CELLIS == $conditional->getConditionType()) {
    ...
    private function writeLabel($row, $col, $str, $xfIndex)
    {
        ...
        if ($strlen > $this->xlsStringMaxLength) { // LABEL must be < 255 chars
            $str = mb_substr($str, 0, $this->xlsStringMaxLength);
    ...
    private function writeNote($row, $col, $note)
    {
        ...
        $this->append($header . $data . mb_substr($note, 0, 2048));

        for ($i = $max_length; $i < $note_length; $i += $max_length) {
            $chunk = mb_substr($note, $i, $max_length);
            $length = 0x0006 + mb_strlen($chunk);
            $header = pack('vv', $record, $length);
            $data = pack('vvv', -1, 0, mb_strlen($chunk));
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
    {
        ...
        try {
            ...
            $formlen = mb_strlen($formula);    // Length of the binary string
    ...
    public function writeUrlWeb($row1, $col1, $row2, $col2, $url)
    {
        ...
        $url_len = pack('V', mb_strlen($url));

        // Calculate the data length
        $length = 0x34 + mb_strlen($url);
    ...
    public function writeUrlExternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $dir_short_len = pack('V', mb_strlen($dir_short));
        $dir_long_len = pack('V', mb_strlen($dir_long));
    ...
    private function writeWindow2()
    {
        ...
        $fPaged = 1;                     // 2
        $fPageBreakPreview = PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW === $this->phpSheet->getSheetView()->getView();
    ...
    private function writeExternsheet($sheetname)
    {
        ...
        if ($this->phpSheet->getTitle() == $sheetname) {
            ...
            $length = 0x02 + mb_strlen($sheetname);
            $cch = mb_strlen($sheetname);
    ...
    private function writeSetup()
    {
        ...
        // Page orientation
        $fLandscape = (PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE == $this->phpSheet->getPageSetup()->getOrientation()) ?
            0x0 : 0x1;
    ...
    private function writeBreaks()
    {
        ...
        if (!empty($hbreaks)) {
            ...
            if (0 == $hbreaks[0]) { // don't use first break if it's 0
                array_shift($hbreaks);
            }
        ...
        if (!empty($vbreaks)) {
            ...
            if (0 == $vbreaks[0]) { // don't use first break if it's 0
                array_shift($vbreaks);
            }
    ...
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        //
        if (0 == PHPExcel_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_start))) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_end))) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1)) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1)) {
            return;
        }
    ...
    public function processBitmapGd($image)
    {
        ...
        return [$width, $height, mb_strlen($data), $data];
    }
    ...
    public function processBitmap($bitmap)
    {
        ...
        //
        $size_array = unpack('Vsa', mb_substr($data, 0, 4));
        $size = $size_array['sa'];
        ...
        $data = mb_substr($data, 12);

        // Read and remove the bitmap width and height. Verify the sizes.
        $width_and_height = unpack('V2', mb_substr($data, 0, 8));
        $width = $width_and_height[1];
        $height = $width_and_height[2];
        $data = mb_substr($data, 8);
        if ($width > 0xFFFF) {
        ...
        $planes_and_bitcount = unpack('v2', mb_substr($data, 0, 4));
        $data = mb_substr($data, 4);
        ...
        $compression = unpack('Vcomp', mb_substr($data, 0, 4));
        $data = mb_substr($data, 4);
    ...
    private function writeZoom()
    {
        ...
        if (100 == $this->phpSheet->getSheetView()->getZoomScale()) {
            return;
        }
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            ...
            for ($i = 1; $i <= $nm; ++$i) {
                ...
                $dataChunk = mb_substr($data, $spOffsets[$i - 1], $spOffsets[$i] - $spOffsets[$i - 1]);

                $length = mb_strlen($dataChunk);
                $header = pack('vv', $record, $length);
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                // explicit formula?
                if (0x03 == $type && preg_match('/^\".*\"$/', $dataValidation->getFormula1())) {
                    $options |= 0x01 << 7;
                }
                ...
                try {
                    ...
                    if (0x03 == $type) { // list type
                        $formula1 = str_replace(',', chr(0), $formula1);
                    }
                    $this->parser->parse($formula1);
                    $formula1 = $this->parser->toReversePolish();
                    $sz1 = mb_strlen($formula1);
                } catch (PHPExcel_Exception $e) {
                    $sz1 = 0;
                    $formula1 = '';
                }
    ...
    private function writeCFRule(PHPExcel_Style_Conditional $conditional)
    {
        ...
        if (1 == $numConditions) {
            ...
            $operand2 = null;
        } elseif (2 == $numConditions && (PHPExcel_Style_Conditional::OPERATOR_BETWEEN == $conditional->getOperatorType())) {
            $szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
        ...
        // $flags : Option flags
        // Alignment
        $bAlignHz = (null == $conditional->getStyle()->getAlignment()->getHorizontal() ? 1 : 0);
        $bAlignVt = (null == $conditional->getStyle()->getAlignment()->getVertical() ? 1 : 0);
        $bAlignWrapTx = (false == $conditional->getStyle()->getAlignment()->getWrapText() ? 1 : 0);
        $bTxRotation = (null == $conditional->getStyle()->getAlignment()->getTextRotation() ? 1 : 0);
        $bIndent = (0 == $conditional->getStyle()->getAlignment()->getIndent() ? 1 : 0);
        $bShrinkToFit = (false == $conditional->getStyle()->getAlignment()->getShrinkToFit() ? 1 : 0);
        if (0 == $bAlignHz || 0 == $bAlignVt || 0 == $bAlignWrapTx || 0 == $bTxRotation || 0 == $bIndent || 0 == $bShrinkToFit) {
            $bFormatAlign = 1;
        } else {
            $bFormatAlign = 0;
        }
        // Protection
        $bProtLocked = (null == $conditional->getStyle()->getProtection()->getLocked() ? 1 : 0);
        $bProtHidden = (null == $conditional->getStyle()->getProtection()->getHidden() ? 1 : 0);
        if (0 == $bProtLocked || 0 == $bProtHidden) {
            $bFormatProt = 1;
        } else {
            $bFormatProt = 0;
        }
        // Border
        $bBorderLeft = (PHPExcel_Style_Color::COLOR_BLACK == $conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB()
                        && PHPExcel_Style_Border::BORDER_NONE == $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() ? 1 : 0);
        $bBorderRight = (PHPExcel_Style_Color::COLOR_BLACK == $conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB()
                        && PHPExcel_Style_Border::BORDER_NONE == $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() ? 1 : 0);
        $bBorderTop = (PHPExcel_Style_Color::COLOR_BLACK == $conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB()
                        && PHPExcel_Style_Border::BORDER_NONE == $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() ? 1 : 0);
        $bBorderBottom = (PHPExcel_Style_Color::COLOR_BLACK == $conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB()
                        && PHPExcel_Style_Border::BORDER_NONE == $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() ? 1 : 0);
        if (0 == $bBorderLeft || 0 == $bBorderRight || 0 == $bBorderTop || 0 == $bBorderBottom) {
            $bFormatBorder = 1;
        } else {
            $bFormatBorder = 0;
        }
        // Pattern
        $bFillStyle = (null == $conditional->getStyle()->getFill()->getFillType() ? 0 : 1);
        $bFillColor = (null == $conditional->getStyle()->getFill()->getStartColor()->getARGB() ? 0 : 1);
        $bFillColorBg = (null == $conditional->getStyle()->getFill()->getEndColor()->getARGB() ? 0 : 1);
        if (0 == $bFillStyle || 0 == $bFillColor || 0 == $bFillColorBg) {
            $bFormatFill = 1;
        } else {
            $bFormatFill = 0;
        }
        // Font
        if (null != $conditional->getStyle()->getFont()->getName()
            || null != $conditional->getStyle()->getFont()->getSize()
            || null != $conditional->getStyle()->getFont()->getBold()
            || null != $conditional->getStyle()->getFont()->getItalic()
            || null != $conditional->getStyle()->getFont()->getSuperScript()
            || null != $conditional->getStyle()->getFont()->getSubScript()
            || null != $conditional->getStyle()->getFont()->getUnderline()
            || null != $conditional->getStyle()->getFont()->getStrikethrough()
            || null != $conditional->getStyle()->getFont()->getColor()->getARGB()) {
            $bFormatFont = 1;
        } else {
            $bFormatFont = 0;
        }
        ...
        if (1 == $bFormatFont) {
            ...
            // Font Size
            if (null == $conditional->getStyle()->getFont()->getSize()) {
            ...
            // Font weight
            if (true == $conditional->getStyle()->getFont()->getBold()) {
                $dataBlockFont .= pack('v', 0x02BC);
            } else {
                $dataBlockFont .= pack('v', 0x0190);
            }
            // Escapement type
            if (true == $conditional->getStyle()->getFont()->getSubScript()) {
                $dataBlockFont .= pack('v', 0x02);
                $fontEscapement = 0;
            } elseif (true == $conditional->getStyle()->getFont()->getSuperScript()) {
            ...
            $optionsFlags = 0;
            $optionsFlagsBold = (null == $conditional->getStyle()->getFont()->getBold() ? 1 : 0);
        ...
        if (1 == $bFormatAlign) {
            ...
            if (true == $conditional->getStyle()->getAlignment()->getWrapText()) {
                $blockAlign |= 1 << 3;
            } else {
                $blockAlign |= 0 << 3;
            }
            ...
            if (true == $conditional->getStyle()->getAlignment()->getShrinkToFit()) {
                $blockIndent |= 1 << 4;
            } else {
                $blockIndent |= 0 << 4;
            }
        ...
        if (1 == $bFormatProt) {
            ...
            if (PHPExcel_Style_Protection::PROTECTION_PROTECTED == $conditional->getStyle()->getProtection()->getLocked()) {
                $dataBlockProtection = 1;
            }
            if (PHPExcel_Style_Protection::PROTECTION_PROTECTED == $conditional->getStyle()->getProtection()->getHidden()) {
                $dataBlockProtection = 1 << 1;
            }
        }

        $data = pack('CCvvVv', $type, $operatorType, $szValue1, $szValue2, $flags, 0x0000);
        if (1 == $bFormatFont) { // Block Formatting : OK
            $data .= $dataBlockFont;
        }
        if (1 == $bFormatAlign) {
            $data .= $dataBlockAlign;
        ...
        if (1 == $bFormatFill) { // Block Formatting : OK
            $data .= $dataBlockFill;
        }
        if (1 == $bFormatProt) {
        ...
        $header = pack('vv', $record, mb_strlen($data));
        $this->append($header . $data);
    }
        ...
        foreach ($this->phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            foreach ($conditionalStyles as $conditional) {
                if (PHPExcel_Style_Conditional::CONDITION_EXPRESSION == $conditional->getConditionType()
                    ...
                    if (null === $numColumnMin || ($numColumnMin > $arrCoord[0])) {
                        $numColumnMin = $arrCoord[0];
                    }
                    if (null === $numColumnMax || ($numColumnMax < $arrCoord[0])) {
                        $numColumnMax = $arrCoord[0];
                    }
                    if (null === $numRowMin || ($numRowMin > $arrCoord[1])) {
                        $numRowMin = $arrCoord[1];
                    }
                    if (null === $numRowMax || ($numRowMax < $arrCoord[1])) {
