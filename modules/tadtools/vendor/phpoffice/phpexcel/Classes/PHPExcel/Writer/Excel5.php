...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            if (0 == count($sheet->getDrawingCollection()) && empty($filterRange)) {
                continue;
            }
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        if ($this->phpExcel->getProperties()->getCategory()) {
            ...
            $dataSection[] = ['summary' => ['pack' => 'V', 'data' => 0x02],
                                   ...
                                   'data' => ['data' => $dataProp, 'length' => mb_strlen($dataProp)], ];
            $dataSection_NumProps++;
        }
        ...
        foreach ($dataSection as $dataProp) {
            ...
            // Data
            if (0x02 == $dataProp['type']['data']) { // 2 byte signed integer
                $dataSection_Content .= pack('V', $dataProp['data']['data']);

                $dataSection_Content_Offset += 4 + 4;
            } elseif (0x03 == $dataProp['type']['data']) { // 4 byte signed integer
                $dataSection_Content .= pack('V', $dataProp['data']['data']);
                ...
                $dataSection_Content_Offset += 4 + 4;
            } elseif (0x1E == $dataProp['type']['data']) { // null-terminated string prepended by dword string length
                // Null-terminated string
                $dataProp['data']['data'] .= chr(0);
                $dataProp['data']['length'] += 1;
                // Complete the string with null string for being a %4
                $dataProp['data']['length'] = $dataProp['data']['length'] + (4 == (4 - $dataProp['data']['length'] % 4) ? 0 : (4 - $dataProp['data']['length'] % 4));
                $dataProp['data']['data'] = str_pad($dataProp['data']['data'], $dataProp['data']['length'], chr(0), STR_PAD_RIGHT);

                $dataSection_Content .= pack('V', $dataProp['data']['length']);
                $dataSection_Content .= $dataProp['data']['data'];

                $dataSection_Content_Offset += 4 + 4 + mb_strlen($dataProp['data']['data']);
            } elseif (0x40 == $dataProp['type']['data']) { // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                $dataSection_Content .= $dataProp['data']['data'];
