...
class PHPExcel_Shared_OLE
{
    ...
    public function _readPpsWks($blockId)
    {
        ...
        for ($pos = 0; ; $pos += 128) {
            ...
            $nameUtf16 = mb_substr($nameUtf16, 0, $nameLength - 2);
        ...
        foreach ($this->_list as $pps) {
            if (self::OLE_PPS_TYPE_DIR == $pps->Type || self::OLE_PPS_TYPE_ROOT == $pps->Type) {
                $nos = [$pps->DirPps];
                $pps->children = [];
                while ($nos) {
                    $no = array_pop($nos);
    ...
    public function isFile($index)
    {
        if (isset($this->_list[$index])) {
            return (self::OLE_PPS_TYPE_FILE == $this->_list[$index]->Type);
        }
    ...
    {
        if (isset($this->_list[$index])) {
            return (self::OLE_PPS_TYPE_ROOT == $this->_list[$index]->Type);
        }
    ...
    {
        if (8 != mb_strlen($string)) {
            return new PEAR_Error('Expecting 8 byte string');
        }

        // factor used for separating numbers into 4 bytes parts
        $factor = pow(2, 32);
        list(, $high_part) = unpack('V', mb_substr($string, 4, 4));
        list(, $low_part) = unpack('V', mb_substr($string, 0, 4));
