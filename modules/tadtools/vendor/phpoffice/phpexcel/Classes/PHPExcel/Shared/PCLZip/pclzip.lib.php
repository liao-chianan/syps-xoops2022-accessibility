...
class PclZip
{
    ...
    public function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        for ($j = 0; ($j < count($p_filedescr_list)) && (1 == $v_result); $j++) {
            ...
            // ----- Check the filename
            if (('virtual_file' != $p_filedescr_list[$j]['type']) && (!file_exists($p_filedescr_list[$j]['filename']))) {
            ...
            if (('file' == $p_filedescr_list[$j]['type']) || ('virtual_file' == $p_filedescr_list[$j]['type']) || (('folder' == $p_filedescr_list[$j]['type']) && (!isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]) || !$p_options[PCLZIP_OPT_REMOVE_ALL_PATH]))) {
                // ----- Add the file
            ...
            if ($p_remove_all_dir) {
                ...
                if ($v_compare > 0) {
                    if (2 == $v_compare) {
                        ...
                        $v_stored_filename = mb_substr($v_stored_filename, mb_strlen($p_remove_dir));
                    }
                }
            }
    ...
    public function privWriteFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack('VvvvvvVVVvv', 0x04034b50, $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], mb_strlen($p_header['stored_filename']), $p_header['extra_len']);
    ...
    public function privWriteCentralFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack('VvvvvvvVVVvvvvvVV', 0x02014b50, $p_header['version'], $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], mb_strlen($p_header['stored_filename']), $p_header['extra_len'], $p_header['comment_len'], $p_header['disk'], $p_header['internal'], $p_header['external'], $p_header['offset']);
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        // ----- Check the path
        if (('' == $p_path) || (('/' != mb_substr($p_path, 0, 1)) && ('../' != mb_substr($p_path, 0, 3)) && (':/' != mb_substr($p_path, 1, 2)))) {
            $p_path = './' . $p_path;
        }
        ...
        for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && (0 != $p_options[PCLZIP_OPT_BY_NAME])) {
                // ----- Look if the filename is in the list
                for ($j = 0; ($j < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
                    // ----- Look for a directory
                    if ('/' == mb_substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1)) {
                        // ----- Look if the directory is in the filename path
                        if ((mb_strlen($v_header['stored_filename']) > mb_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (mb_substr($v_header['stored_filename'], 0, mb_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_extract = true;
                        }
            ...
            } elseif ((isset($p_options[PCLZIP_OPT_BY_INDEX])) && (0 != $p_options[PCLZIP_OPT_BY_INDEX])) {
            ...
            if (($v_extract) && ((8 != $v_header['compression']) && (0 != $v_header['compression']))) {
                $v_header['status'] = 'unsupported_compression';

                // ----- Look for PCLZIP_OPT_STOP_ON_ERROR
                if ((isset($p_options[PCLZIP_OPT_STOP_ON_ERROR])) && (true === $p_options[PCLZIP_OPT_STOP_ON_ERROR])) {
                    $this->privSwapBackMagicQuotes();
        ...
        if ('ok' == $p_entry['status']) {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if (is_dir($p_entry['filename'])) {
                    ...
                    // ----- Change the file status
                    if ((isset($p_options[PCLZIP_OPT_REPLACE_NEWER])) && (true === $p_options[PCLZIP_OPT_REPLACE_NEWER])) {
                    } else {
    ...
    public function privDeleteByRule(&$p_result_list, &$p_options)
    {
        ...
        for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && (0 != $p_options[PCLZIP_OPT_BY_NAME])) {
                ...
                for ($j = 0; ($j < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
                    ...
                    if ('/' == mb_substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1)) {
                        // ----- Look if the directory is in the filename path
                        if ((mb_strlen($v_header_list[$v_nb_extracted]['stored_filename']) > mb_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (mb_substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, mb_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
                        } elseif ((0x00000010 == ($v_header_list[$v_nb_extracted]['external'] & 0x00000010)) /* Indicates a folder */ && ($v_header_list[$v_nb_extracted]['stored_filename'] . '/' == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
                        }
        ...
        if ($v_nb_extracted > 0) {
            ...
            for ($i = 0; $i < count($v_header_list); $i++) {
                ...
                if (1 != $this->privCheckFileHeaders($v_local_header, $v_header_list[$i])) {
                    // TBC
                }
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if (('.' == $p_dir) || ((mb_strlen($p_dir) >= 2) && ('./' == mb_substr($p_dir, 0, 2)))) {
        $p_dir = PclZipUtilTranslateWinPath(getcwd(), false) . '/' . mb_substr($p_dir, 1);
    }
    if (('.' == $p_path) || ((mb_strlen($p_path) >= 2) && ('./' == mb_substr($p_path, 0, 2)))) {
...
function PclZipUtilOptionText($p_option)
{
    ...
    for (reset($v_list); $v_key = key($v_list); next($v_list)) {
        ...
        if ((('PCLZIP_OPT' == $v_prefix) || ('PCLZIP_CB_' == $v_prefix) || ('PCLZIP_ATT' == $v_prefix)) && ($v_list[$v_key] == $p_option)) {
            return $v_key;
        }
    }
