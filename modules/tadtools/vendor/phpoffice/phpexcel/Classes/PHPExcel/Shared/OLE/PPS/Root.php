...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
    ...
    public function _calcSize(&$raList)
    {
        ...
        $iCount = count($raList);
        for ($i = 0; $i < $iCount; ++$i) {
            if (PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE == $raList[$i]->Type) {
    ...
    public function _saveBigData($iStBlk, &$raList)
    {
        ...
        $iCount = count($raList);
        for ($i = 0; $i < $iCount; ++$i) {
            if (PHPExcel_Shared_OLE::OLE_PPS_TYPE_DIR != $raList[$i]->Type) {
                $raList[$i]->Size = $raList[$i]->_DataLen();
                if (($raList[$i]->Size >= PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL) || ((PHPExcel_Shared_OLE::OLE_PPS_TYPE_ROOT == $raList[$i]->Type) && isset($raList[$i]->_data))) {
                    // Write Data
