...
class PasswordEncoder
{
    ...
    public static function hashPassword($password, $algorithmName = self::ALGORITHM_SHA_1, $salt = null, $spinCount = 10000)
    {
        ...
        for ($i = 0; $i < mb_strlen($password); $i++) {
            $byteChars[$i] = ord(mb_substr($passUtf8, $i * 2, 1));

            if (0 == $byteChars[$i]) {
                $byteChars[$i] = ord(mb_substr($passUtf8, $i * 2 + 1, 1));
            }
        }
        ...
        $hex = str_pad(mb_strtoupper(dechex($combinedKey & 0xFFFFFFFF)), 8, '0', \STR_PAD_LEFT);
