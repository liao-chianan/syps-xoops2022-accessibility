...
class pdf_chinese extends PDF_MC_Table
{
    ...
    public function MBMultiCell($w, $h, $txt, $border = 0, $align = 'L', $fill = 0)
    {
        ...
        while ($i < $nb) {
            ...
            if ("\n" == $c) {
                //Explicit line break
                parent::Cell($w, $h, mb_substr($s, $j, $i - $j), $b, 2, $align, $fill);
                $i++;
                $sep = -1;
                $j = $i;
                $l = 0;
            ...
            if ($l > $wmax) {
                ...
                if (-1 == $sep or $i == $j) {
                    ...
                    parent::Cell($w, $h, mb_substr($s, $j, $sep - $j), $b, 2, $align, $fill);
                    $i = (' ' == $s[$sep]) ? $sep + 1 : $sep;
                }
                $sep = -1;
                $j = $i;
    ...
    public function MBWrite($h, $txt, $link)
    {
        ...
        while ($i < $nb) {
            ...
            if ("\n" == $c) {
                //Explicit line break
                parent::Cell($w, $h, mb_substr($s, $j, $i - $j), 0, 2, '', 0, $link);
                $i++;
                $sep = -1;
                $j = $i;
                $l = 0;
            ...
            if ($l > $wmax) {
                ...
                if (-1 == $sep or $i == $j) {
                    ...
                    parent::Cell($w, $h, mb_substr($s, $j, $sep - $j), 0, 2, '', 0, $link);
                    $i = (' ' == $s[$sep]) ? $sep + 1 : $sep;
                }
                $sep = -1;
                $j = $i;
        ...
        //Last chunk
        if ($i != $j) {
            parent::Cell($l / 1000 * $this->FontSize, $h, mb_substr($s, $j, $i - $j), 0, 0, '', 0, $link);
        }
    }
    ...
    public function _putType0($font)
    {
        ...
        if ('ETen-B5-H' == $font['CMap']) {
            $W = '13648 13742 500';
        } elseif ('GBK-EUC-H' == $font['CMap']) {
