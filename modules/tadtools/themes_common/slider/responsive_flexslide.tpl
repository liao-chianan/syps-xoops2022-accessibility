...
if (file_exists(XOOPS_ROOT_PATH . "/modules/tadtools/flexslider2.php")) {
    require XOOPS_ROOT_PATH . "/modules/tadtools/flexslider2.php";
    $flexslider = new flexslider(120, true);
        ...
        while (list($menuid, $itemname, $itemurl) = $xoopsDB->fetchRow($result)) {
            ...
            if (strpos($now_url, $patten_arr[0]) !== false and (preg_match("/{$patten_arr[1]}&/", $now_url) or preg_match("/{$patten_arr[1]}$/", $now_url))) {
                if (file_exists(XOOPS_ROOT_PATH . "/uploads/tad_themes/menu_banner/{$menuid}.png")) {
                    $flexslider->add_content(1, '', "", XOOPS_URL . "/uploads/tad_themes/menu_banner/{$menuid}.png", "", $itemurl);
    ...
    if ($no_item_slide_images) {
        ...
        if (empty($slide_images)) {
            ...
            $flexslider->add_content(1, $title, $content, XOOPS_URL . "/themes/{$xoopsConfig['theme_set']}/images/slide/default.png", $date, XOOPS_URL);
            $flexslider->add_content(2, $title, $content, XOOPS_URL . "/themes/{$xoopsConfig['theme_set']}/images/slide/default2.png", $date, XOOPS_URL);
            $flexslider->add_content(3, $title, $content, XOOPS_URL . "/themes/{$xoopsConfig['theme_set']}/images/slide/default3.png", $date, XOOPS_URL);
        }
    }
    ...
    $slide_images = $flexslider->render();
    echo $slide_images;
}
<{/php}>
