...
function get_config2_values($theme_id = '')
{
    global $xoopsDB;
    $values = [];
    $sql = 'select `name`, `value` from ' . $xoopsDB->prefix('tad_themes_config2') . " where `theme_id` = '{$theme_id}'";
    $result = $xoopsDB->query($sql) or Utility::web_error($sql, __FILE__, __LINE__);
            ...
            foreach ($setup_items as $label => $value) {
                ...
                if (is_array($value)) {
                    ...
                    foreach ($value as $k => $v) {
                        ...
                        } elseif (strpos($v, XOOPS_URL) !== false) {
...
switch ($op) {
    ...
    case 'insert_tad_themes':
        ...
        // 在save_config2()中已經刪了
        // unlink(XOOPS_VAR_PATH . "/data/tad_themes_config2_{$theme_id}.json");
