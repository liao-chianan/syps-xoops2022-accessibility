...
function ugm_page_b_cate($options){
	global $xoopsDB;
  ...
  //-------------------------------------------------------------------
  $css="<link rel='stylesheet' type='text/css' href='{$path}/class/page_b_cate/ddaccordion.css' />" ;
  $block="
    {$jquery_path}
    <script type='text/javascript' src='{$path}/class/page_b_cate/ddaccordion.js'>
      /***********************************************
    ...
    {$css}
    <div class='ugm_page_b_cate' style='width: {$options[2]}px;'>
    ";
   //不考慮多層cate 
  $and_key="";
  $and_or=0;
  ...
  $sql = "select *  from ".$xoopsDB->prefix("ugm_page_cate")." where `enable`=1 {$and_key} order by `sort`";
    ...
    //get文章
    $block_body=get_main_body($csn,$options[1],$options[5]);
    if(empty($block_body)){
      //沒有文章
...
//區塊編輯函式
function ugm_page_b_cate_edit($options){
  $options[0]=(empty($options[0]))?strtotime("now"):$options[0];//ID  
  $options[1]=(empty($options[1]))?0:$options[1];//文章數
  $options[2]=(empty($options[2]))?180:$options[2];//寬度
  $options[3]=(empty($options[3]))?"click":$options[3];//滑鼠點擊樣式 'click', or 'clickgo' or 'mouseover' 
...
#
###############################################################################
if(!function_exists("get_main_body")){
function get_main_body($csn=0,$counter=0,$sort){
  global $xoopsDB;
