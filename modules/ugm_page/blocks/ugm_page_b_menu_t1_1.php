...
function ugm_page_b_menu_t1_1($options){
	global $xoopsDB;
	$options[0]=(empty($options[0]))?strtotime("now"):$options[0];//ID  
  $options[1]=(empty($options[1]))?"":$options[1];//文章數
  $options[2]=(empty($options[2]))?"":$options[2];//寬度
  $options[3]=(empty($options[3]))?"click":$options[3];//滑鼠點擊樣式 'click', or 'clickgo' or 'mouseover'
  $path=XOOPS_URL."/modules/ugm_page/class/menu_t1_1";
  $width_style=empty($options[2])?"":" style='width: {$options[2]}px;'";
	//--------------------- 引入jquery -------------------------------------
  ...
  $block="
    ...
    {$css}
    <div class='ugm_page_menu_t1_1' {$width_style}>
    ";
   //不考慮多層cate 
  ...
  $sql = "select *  from ".$xoopsDB->prefix("ugm_page_menu")." where `menu_enable`=1 and `menu_type`=1 and `menu_ofsn`='{$options[4]}' order by `menu_sort`";
    ...
    //get得到第一層
    $block_body=get_b_menu_t1_1_body($menu_sn,$options[1]);
...
//區塊編輯函式
function ugm_page_b_menu_t1_1_edit($options){
  $options[0]=(empty($options[0]))?strtotime("now"):$options[0];//ID  
  $options[1]=(empty($options[1]))?"":$options[1];//選單數 ;「0」為不限制
  $options[2]=(empty($options[2]))?"":$options[2];//寬度
  $options[3]=(empty($options[3]))?"click":$options[3];//滑鼠點擊樣式 'click', or 'clickgo' or 'mouseover'
  ...
  <table style='width:auto;'> 
    ...
    <tr><th>2.</th><th>"._MB_UGMPAGE_B_MENU_T1_OP1."</th><td><INPUT type='text' name='options[1]' value='{$options[1]}' size=3> "._MB_UGMPAGE_B_MENU_T1_OP1_R."</td></tr>
      ...
      ".get_ugmpage_menu_t1_1_cate(1,0,$options[4])."
      </select></td></tr>
  </table>
  ";
	return $form;
...
if(!function_exists("get_ugmpage_menu_t1_1_cate")){
function get_ugmpage_menu_t1_1_cate($menu_type=0,$menu_ofsn=0,$menu_sn_chk=0){
...
#
###############################################################################
if(!function_exists("get_b_menu_t1_1_body")){
function get_b_menu_t1_1_body($menu_ofsn=0,$counter=0){
  global $xoopsDB;
